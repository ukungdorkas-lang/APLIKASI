...
import { Calendar, MapPin, User, Truck, ArrowLeft, Share2, AlertCircle, Newspaper, Bookmark } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto px-6 sm:px-10">
        ...
        {article.photos && article.photos.length > 0 && (
          <motion.div 
            ...
            className="mb-20 rounded-[3rem] overflow-hidden shadow-2xl relative group"
          >
            ...
            {article.photos && article.photos.length > 1 && (
              <div className="grid grid-cols-2 gap-6 mt-20 not-prose">
                {article.photos.slice(1).map((photo, i) => (
                  <div key={i} className="rounded-3xl overflow-hidden aspect-[4/3] shadow-lg border border-slate-50">
                    <img src={photo} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" alt={`img-${i}`} />
                  </div>
                ))}
              </div>
            )}
