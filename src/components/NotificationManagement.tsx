...
import { IncidentType, NotificationRecipient } from '../types';

export default function NotificationManagement() {
  const { recipients, logs, loading, addRecipient, updateRecipient, deleteRecipient } = useNotifications();
  const [showAddModal, setShowAddModal] = useState(false);
  const [activeSubTab, setActiveSubTab] = useState<'recipients' | 'logs'>('recipients');
  
  const [newRecipient, setNewRecipient] = useState<Omit<NotificationRecipient, 'id' | 'createdAt'>>({
    name: '',
    phoneNumber: '',
    email: '',
    divisi: 'Operator',
  ...
  const incidentTypes: IncidentType[] = ['Kebakaran', 'Evakuasi', 'Penyelamatan', 'Pohon Tumbang', 'Hewan Berbahaya', 'Banjir', 'Lainnya'];
      ...
      <div className="flex gap-4">
        <button 
          ...
          className={cn(
            ...
            activeSubTab === 'recipients' ? "bg-slate-900 text-white shadow-xl shadow-slate-200" : "bg-white text-slate-400"
          )}
        >
        ...
        <button 
          ...
          className={cn(
            "px-8 py-3 rounded-xl tag-label transition-all flex items-center gap-2",
            activeSubTab === 'logs' ? "bg-slate-900 text-white shadow-xl shadow-slate-200" : "bg-white text-slate-400"
          )}
        >
      ...
      {activeSubTab === 'recipients' ? (
        <div className="space-y-8">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-3xl heading-bold text-brand-dark">Penerima <span className="text-brand-red">Terdaftar</span></h3>
              <p className="tag-label text-slate-400">Pemberitahuan otomatis akan dikirim ke nomor ini</p>
            </div>
            <button 
              onClick={() => setShowAddModal(true)}
              className="px-8 py-4 bg-brand-red text-white flex items-center gap-3 rounded-xl heading-bold text-sm shadow-xl shadow-rose-200 hover:scale-105 transition-all"
            >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {recipients.map((recipient) => (
              ...
              >
                {!recipient.isActive && <div className="absolute inset-0 bg-slate-50/80 backdrop-blur-[2px] z-10 flex items-center justify-center">
                  <span className="px-6 py-2 bg-slate-900 text-white rounded-full text-[10px] font-black uppercase tracking-widest">NONAKTIF</span>
                </div>}

                <div className="flex justify-between items-start mb-6">
                  <div className="w-14 h-14 bg-slate-900 rounded-2xl flex items-center justify-center font-black text-white italic text-xl">
                    {recipient.name?.[0] || '?'}
                  </div>
                  <div className="flex gap-2">
                    <button 
                      onClick={() => updateRecipient(recipient.id, { isActive: !recipient.isActive })}
                ...
                <h4 className="text-xl heading-bold leading-none mb-1">{recipient.name}</h4>
                <p className="text-[10px] font-black text-brand-red uppercase tracking-widest mb-4 italic">{recipient.divisi}</p>
                
                <div className="space-y-3 mb-6">
                  <div className="flex items-center gap-3 text-slate-500">
                    <Phone className="w-4 h-4" />
                    <span className="text-sm font-bold">{recipient.phoneNumber}</span>
                  </div>
                  <div className="flex gap-2">
                    {recipient.channels.map(ch => (
                      <span key={ch} className="w-8 h-8 rounded-lg bg-slate-50 border-2 border-slate-100 flex items-center justify-center text-slate-400">
                ...
                <div className="flex flex-wrap gap-2 pt-6 border-t-2 border-slate-50">
                  {recipient.categories.map(cat => (
                    <span key={cat} className="text-[8px] font-black uppercase tracking-widest bg-slate-900 text-white px-2 py-1 rounded">
                      {cat}
                    </span>
                  ))}
                </div>
              ...
              <tbody className="divide-y-2 divide-slate-50">
                 {logs.map(log => (
                   <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-8">
                         <p className="font-black italic uppercase tracking-tighter">{log.recipientName}</p>
                         <p className="text-[10px] font-bold text-slate-400">{log.phoneNumber}</p>
                      </td>
                      <td className="p-8">
                         <span className="flex items-center gap-2 tag-label text-[10px]">
                           ...
                           {log.channel === 'sms' && <Smartphone className="w-3 h-3 text-blue-500" />}
                      ...
                      <td className="p-8 max-w-xs">
                         <p className="text-[10px] font-bold text-slate-500 line-clamp-1 italic">"{log.messageContent}"</p>
                      </td>
                   </tr>
                 ))}
              </tbody>
            ...
            >
              <form onSubmit={handleSubmit} className="p-12 space-y-8">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h3 className="text-3xl heading-bold text-brand-dark uppercase tracking-tighter italic">Penerima Baru</h3>
                </div>

                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div>
                      <label className="tag-label text-slate-500 mb-2 block">Nama Lengkap</label>
                      <input 
                        required
                    ...
                    <div>
                      <label className="tag-label text-slate-500 mb-2 block">Nomor HP / WhatsApp</label>
                      <input 
                        required
                        type="tel"
                        className="w-full bg-slate-50 border-4 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-brand-red"
                    ...
                    <div>
                      <label className="tag-label text-slate-500 mb-2 block">Divisi / Jabatan</label>
                      <select 
                        className="w-full bg-slate-50 border-4 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-brand-red"
                        value={newRecipient.divisi}
                        onChange={e => setNewRecipient({...newRecipient, divisi: e.target.value})}
                      >
                  ...
                  <div className="space-y-6">
                    <div>
                      <label className="tag-label text-slate-500 mb-2 block">Channel Notifikasi</label>
                        ...
                        {channels.map(ch => (
                          <button
                            ...
                            className={cn(
                              ...
                              newRecipient.channels.includes(ch) ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-400 border-slate-100"
                            )}
                          >
                            {ch.toUpperCase()}
                          </button>
                    ...
                    <div>
                      <label className="tag-label text-slate-500 mb-2 block">Kategori Kejadian</label>
                      <div className="flex flex-wrap gap-2">
                        {incidentTypes.map(it => (
                          <button
                            ...
                            className={cn(
                              ...
                              newRecipient.categories.includes(it) ? "bg-brand-red text-white border-brand-red" : "bg-white text-slate-400 border-slate-100"
                            )}
                          >
                            {it}
                          </button>
